...
''' PERCENTAGE '''
df = pd.read_excel('updated_toxicity_analysis_percentage_arrmay.xlsx')
...
formulas = [
    'perc_toxic_all ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'perc_toxic_pris ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'perc_toxic_guard ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)'
]
...
new_variable_names = {
    ...
    'Research_Oversight': 'Discl. Research Oversight',
    'C(goal)[T.Yard Time]': 'Goal: Yard Time',
    'C(personality_prisoner)[T.Peaceful Prisoner]': 'Pris. Pers.: Peaceful',
    'C(personality_prisoner)[T.Rebellious Prisoner]': 'Pris. Pers: Rebellious',
    'C(personality_guard)[T.Respectful Guard]': 'Guard Pers.: Respectful',
    'C(personality_guard)[T.Abusive Guard]': 'Guard Pers.: Abusive'
}
...
def plot_coefficients(ax, df, title, ylabel=False):
    ...
    for i, (coef, err, color) in enumerate(zip(coefficients, zip(*error_bars), colors)):
        ax.errorbar(coef, i, xerr=[[err[0]], [err[1]]], fmt='o', capsize=5, markersize=2, color=color)
    ...
    ax.set_xlabel('Effect on % Toxic Msgs', fontsize=14)  # Increased x-axis label size

    if ylabel:
        ax.set_yticks(range(len(renamed_variables)))
        ax.set_yticklabels(renamed_variables.values, fontsize=14)  # Increased y-tick labels size
        ax.set_ylabel('', fontsize=12)
    else:
        ax.set_yticks(range(len(renamed_variables)))
        ax.set_yticklabels([''] * len(renamed_variables))  

    # Set x ticks
    ax.set_xticks([-0.4, -0.2, 0, 0.2, 0.4])
    ax.tick_params(axis='both', which='major', labelsize=14)  # Increase tick label sizes
...
plot_coefficients(ax1, summary_df_all, 'Toxicity: Overall', ylabel=True)
# Plot for summary_df_pris
plot_coefficients(ax2, summary_df_pris, 'Toxicity: Prisoner', ylabel=False)
# Plot for summary_df_guard
plot_coefficients(ax3, summary_df_guard, 'Toxicity: Guard', ylabel=False)
...
fig.savefig('toxygen_regression_output_percentage.pdf')
...
corr_matrix = df[['perc_toxic_all', 'perc_toxic_pris', 'perc_toxic_guard']].corr()

# Rename variables for the plot
corr_matrix.index = ['Toxicity: Overall', 'Toxicity: Prisoner', 'Toxicity: Guard']
corr_matrix.columns = ['Toxicity: Overall', 'Toxicity: Prisoner', 'Toxicity: Guard']

# Create a heatmap using seaborn
plt.figure(figsize=(6, 5))
sns.heatmap(corr_matrix, annot=False, cmap='coolwarm', vmin=-1, vmax=1, fmt='.2f', annot_kws={'size': 12})
# Save the figure as a PDF file
plt.savefig('toxygen_correlogram_percentage.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
# Get unique combinations of goal, personality_prisoner, and personality_guard
unique_combinations = df[['goal', 'personality_prisoner', 'personality_guard']].drop_duplicates()

# Separate unique combinations by goal
yard_time_combinations = unique_combinations[unique_combinations['goal'] == 'Yard Time']
escape_combinations = unique_combinations[unique_combinations['goal'] == 'Escape']
...
for i in range(num_combinations):
    ...
    ax_left.bar(index, yard_grouped_means['perc_toxic_all'], bar_width, 
                yerr=yard_grouped_std['perc_toxic_all'], alpha=0.6, color='b', label='Toxicity: Overall')
    ax_left.bar([p + bar_width for p in index], yard_grouped_means['perc_toxic_pris'], bar_width, 
                yerr=yard_grouped_std['perc_toxic_pris'], alpha=0.6, color='g', label='Toxicity: Prisoner')
    ax_left.bar([p + 2 * bar_width for p in index], yard_grouped_means['perc_toxic_guard'], bar_width, 
                yerr=yard_grouped_std['perc_toxic_guard'], alpha=0.6, color='r', label='Toxicity: Guard')
    ...
    if i == num_rows // 2:
        ax_left.set_ylabel('Avg. % Toxic Messages (with Std. Dev.)', size=18)
    ...
    ax_right.bar(index, escape_grouped_means['perc_toxic_all'], bar_width, 
                 yerr=escape_grouped_std['perc_toxic_all'], alpha=0.6, color='b', label='Toxicity: Overall')
    ax_right.bar([p + bar_width for p in index], escape_grouped_means['perc_toxic_pris'], bar_width, 
                 yerr=escape_grouped_std['perc_toxic_pris'], alpha=0.6, color='g', label='Toxicity: Prisoner')
    ax_right.bar([p + 2 * bar_width for p in index], escape_grouped_means['perc_toxic_guard'], bar_width, 
                 yerr=escape_grouped_std['perc_toxic_guard'], alpha=0.6, color='r', label='Toxicity: Guard')
...
fig.savefig('toxygen_barcharts_percentages.pdf', bbox_inches='tight', pad_inches=0.1) 
plt.show()
...
df2 = pd.read_excel('updated_toxicity_analysis_scores_arrmay.xlsx')
...
# Define the formula for the OLS model
formulas = [
    'score_toxic_all ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'score_toxic_pris ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'score_toxic_guard ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)'
]
...
# Fit and save model summaries to DataFrames
model_summaries_scores = []
for formula in formulas:
    model = smf.ols(formula=formula, data=df2).fit()
    summary_df_score = model.summary2().tables[1]
    model_summaries_scores.append(summary_df_score)

# Assign DataFrames to variables for easier access
summary_df_all_score = model_summaries_scores[0]
summary_df_pris_score = model_summaries_scores[1]
summary_df_guard_score = model_summaries_scores[2]

# Display the DataFrame for the first model as a check
print(summary_df_all_score)
...
def plot_coefficients(ax, df, title, ylabel=False):
    ...
    ax.set_xlim([-0.45, 0.45])  # Set same x-axis limit for all subplots
    ax.set_title(title)
    ...
    # Set x ticks
    ax.set_xticks([-0.4, -0.2, 0,  0.2,  0.4])
    ax.set_xlabel('Effect on Toxicity')
...
# Plot for summary_df_all
plot_coefficients(ax1, summary_df_all_score, 'Toxicity: Overall', ylabel=True)
# Plot for summary_df_pris
plot_coefficients(ax2, summary_df_pris_score, 'Toxicity: Prisoner')
# Plot for summary_df_guard
plot_coefficients(ax3, summary_df_guard_score, 'Toxicity: Guard')

# Adjust layout and show the plot
plt.tight_layout()
# Save the figure as a PDF file
fig.savefig('toxygen_regression_output_scores.pdf')
plt.show()
...
import seaborn as sns
# Calculate pairwise correlations
corr_matrix = df2[['score_toxic_all', 'score_toxic_pris', 'score_toxic_guard']].corr()

# Rename variables for the plot
corr_matrix.index = ['Avg. Toxicity: Overall', 'Avg. Toxicity: Prisoner', 'Avg. Toxicity: Guard']
corr_matrix.columns = ['Avg. Toxicity: Overall', 'Avg. Toxicity: Prisoner', 'Avg. Toxicity: Guard']
...
# Save the figure as a PDF file
plt.savefig('toxygen_correlogram_score.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
# Get unique combinations of goal, personality_prisoner, and personality_guard
unique_combinations = df2[['goal', 'personality_prisoner', 'personality_guard']].drop_duplicates()
...
for i in range(num_combinations):
    ...
    # Plotting the Yard Time subplot
    ax_left = axs[row, col_left] if num_combinations > 1 else axs
    ax_left.bar(index, yard_grouped_means['score_toxic_all'], bar_width, 
                yerr=yard_grouped_std['score_toxic_all'], alpha=0.6, color='b', label='Toxicity: Overall')
    ax_left.bar([p + bar_width for p in index], yard_grouped_means['score_toxic_pris'], bar_width, 
                yerr=yard_grouped_std['score_toxic_pris'], alpha=0.6, color='g', label='Toxicity: Prisoner')
    ax_left.bar([p + 2 * bar_width for p in index], yard_grouped_means['score_toxic_guard'], bar_width, 
                yerr=yard_grouped_std['score_toxic_guard'], alpha=0.6, color='r', label='Toxicity: Guard')
    ...
    # Set y-axis label only for the first plot in the left column
    if i == num_rows // 2:
        ax_left.set_ylabel('Avg. Toxicity (with Std. Dev.)', size=18)
        ax_left.yaxis.set_label_coords(-0.15, 0.5)  # Adjust position of y-axis label
    
    # Plotting the Escape subplot
    ax_right = axs[row, col_right] if num_combinations > 1 else axs
    ax_right.bar(index, escape_grouped_means['score_toxic_all'], bar_width, 
                 yerr=escape_grouped_std['score_toxic_all'], alpha=0.6, color='b', label='Toxicity: Overall')
    ax_right.bar([p + bar_width for p in index], escape_grouped_means['score_toxic_pris'], bar_width, 
                 yerr=escape_grouped_std['score_toxic_pris'], alpha=0.6, color='g', label='Toxicity: Prisoner')
    ax_right.bar([p + 2 * bar_width for p in index], escape_grouped_means['score_toxic_guard'], bar_width, 
                 yerr=escape_grouped_std['score_toxic_guard'], alpha=0.6, color='r', label='Toxicity: Guard')
...
# Show the plot
plt.tight_layout(rect=[0.05, 0, 1, 0.96])  
fig.savefig('toxygen_barcharts_score.pdf', bbox_inches='tight', pad_inches=0.1) 
plt.show()
