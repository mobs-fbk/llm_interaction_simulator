...
formulas = [
    'perc_toxic_overall_harassment ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'perc_toxic_pris_harassment ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'perc_toxic_guard_harassment ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)'
]
...
def plot_coefficients(ax, df, title, ylabel=False):
    ...
    ax.set_xlim([-0.55, 0.55])  # Set same x-axis limit for all subplots
    ax.set_title(title)
    ...
    ax.set_xlabel('Effect on % Harassment Messages')
...
plot_coefficients(ax1, summary_df_all, 'Harassment: Overall', ylabel=True)
# Plot for summary_df_pris
plot_coefficients(ax2, summary_df_pris, 'Harassment: Prisoner')
# Plot for summary_df_guard
plot_coefficients(ax3, summary_df_guard, 'Harassment: Guard')

# Adjust layout and show the plot
plt.tight_layout()
# Save the figure as a PDF file
fig.savefig('openai_harassment_regression_output_percentage.pdf')
plt.show()
...
corr_matrix = df[['perc_toxic_overall_harassment', 'perc_toxic_pris_harassment', 'perc_toxic_guard_harassment']].corr()

# Rename variables for the plot
corr_matrix.index = ['% Harassment: Overall', '% Harassment: Prisoner', '% Harassment: Guard']
corr_matrix.columns = ['% Harassment: Overall', '% Harassment: Prisoner', '% Harassment: Guard']

# Create a heatmap using seaborn
plt.figure(figsize=(6, 5))
sns.heatmap(corr_matrix, annot=False, cmap='coolwarm', vmin=-1, vmax=1, fmt='.2f', annot_kws={'size': 8})
# Rotate tick labels
plt.xticks(rotation=00, size=8)
plt.yticks(rotation=90, size=8) 
# Save the figure as a PDF file
plt.savefig('openai_harassment_correlogram_percentage.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
for i in range(num_combinations):
    ...
    ax_left.bar(index, yard_grouped_means['perc_toxic_overall_harassment'], bar_width, 
                yerr=yard_grouped_std['perc_toxic_overall_harassment'], alpha=0.6, color='b', label='Harassment: Overall')
    ax_left.bar([p + bar_width for p in index], yard_grouped_means['perc_toxic_pris_harassment'], bar_width, 
                yerr=yard_grouped_std['perc_toxic_pris_harassment'], alpha=0.6, color='g', label='Harassment: Prisoner')
    ax_left.bar([p + 2 * bar_width for p in index], yard_grouped_means['perc_toxic_guard_harassment'], bar_width, 
                yerr=yard_grouped_std['perc_toxic_guard_harassment'], alpha=0.6, color='r', label='Harassment: Guard')
    ...
    if i == num_rows // 2:
        ax_left.set_ylabel('Avg. % Harassment Messages (with Std. Dev.)', size=18)
    ...
    ax_right.bar(index, escape_grouped_means['perc_toxic_overall_harassment'], bar_width, 
                 yerr=escape_grouped_std['perc_toxic_overall_harassment'], alpha=0.6, color='b', label='Harassment: Overall')
    ax_right.bar([p + bar_width for p in index], escape_grouped_means['perc_toxic_pris_harassment'], bar_width, 
                 yerr=escape_grouped_std['perc_toxic_pris_harassment'], alpha=0.6, color='g', label='Harassment: Prisoner')
    ax_right.bar([p + 2 * bar_width for p in index], escape_grouped_means['perc_toxic_guard_harassment'], bar_width, 
                 yerr=escape_grouped_std['perc_toxic_guard_harassment'], alpha=0.6, color='r', label='Harassment: Guard')
...
fig.savefig('openai_harassment_barcharts_percentages.pdf', bbox_inches='tight', pad_inches=0.1) 
plt.show()
...
# Define the formula for the OLS model
formulas = [
    'score_toxic_overall_harassment ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'score_toxic_pris_harassment ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)',
    'score_toxic_guard_harassment ~ C(llm) + Risks + Research_Oversight + C(goal) + C(personality_prisoner) + C(personality_guard)'
]
...
def plot_coefficients(ax, df, title, ylabel=False):
    ...
    ax.set_xlim([-0.5, 0.5])  # Set same x-axis limit for all subplots
    ax.set_title(title)
    ax.set_xlabel('Coefficient')
    ...
    # Set x ticks
    ax.set_xticks([-0.4, -0.2, 0, 0.2, 0.4])
    ax.set_xlabel('Effect on Harassment')
...
# Plot for summary_df_all
plot_coefficients(ax1, summary_df_all, 'Avg. Harassment: Overall', ylabel=True)
# Plot for summary_df_pris
plot_coefficients(ax2, summary_df_pris, 'Avg. Harassment: Prisoner')
# Plot for summary_df_guard
plot_coefficients(ax3, summary_df_guard, 'Avg. Harassment: Guard')

# Adjust layout and show the plot
plt.tight_layout()
# Save the figure as a PDF file
fig.savefig('openai_harassment_regression_output_scores.pdf')
plt.show()


''' correlogram '''

# Calculate pairwise correlations for the specified columns
corr_matrix = df2[['score_toxic_overall_harassment', 'score_toxic_pris_harassment', 'score_toxic_guard_harassment']].corr()
...
# Save the figure as a PDF file
plt.savefig('openai_harassment_correlogram_score.pdf', format='pdf', bbox_inches='tight')
...
for i in range(num_combinations):
    ...
    # Plotting the Yard Time subplot
    ax_left = axs[row, col_left] if num_combinations > 1 else axs
    ax_left.bar(index, yard_grouped_means['score_toxic_overall_harassment'], bar_width, 
                yerr=yard_grouped_std['score_toxic_overall_harassment'], alpha=0.6, color='b', label='Harassment: Overall')
    ax_left.bar([p + bar_width for p in index], yard_grouped_means['score_toxic_pris_harassment'], bar_width, 
                yerr=yard_grouped_std['score_toxic_pris_harassment'], alpha=0.6, color='g', label='Harassment: Prisoner')
    ax_left.bar([p + 2 * bar_width for p in index], yard_grouped_means['score_toxic_guard_harassment'], bar_width, 
                yerr=yard_grouped_std['score_toxic_guard_harassment'], alpha=0.6, color='r', label='Harassment: Guard')
    ...
    # Set y-axis label only for the first plot in the left column
    if i == num_rows // 2:
        ax_left.set_ylabel('Avg. Harassment (with Std. Dev.)', size=18)
        ax_left.yaxis.set_label_coords(-0.15, 0.5)  # Adjust position of y-axis label
    
    # Plotting the Escape subplot
    ax_right = axs[row, col_right] if num_combinations > 1 else axs
    ax_right.bar(index, escape_grouped_means['score_toxic_overall_harassment'], bar_width, 
                 yerr=escape_grouped_std['score_toxic_overall_harassment'], alpha=0.6, color='b', label='Harassment: Overall')
    ax_right.bar([p + bar_width for p in index], escape_grouped_means['score_toxic_pris_harassment'], bar_width, 
                 yerr=escape_grouped_std['score_toxic_pris_harassment'], alpha=0.6, color='g', label='Harassment: Prisoner')
    ax_right.bar([p + 2 * bar_width for p in index], escape_grouped_means['score_toxic_guard_harassment'], bar_width, 
                 yerr=escape_grouped_std['score_toxic_guard_harassment'], alpha=0.6, color='r', label='Harassment: Guard')
...
# Show the plot
plt.tight_layout(rect=[0.05, 0, 1, 0.96])  # Adjust layout to make room for column titles
fig.savefig('openai_harassment_barcharts_scores.pdf', bbox_inches='tight', pad_inches=0.1) 
plt.show()