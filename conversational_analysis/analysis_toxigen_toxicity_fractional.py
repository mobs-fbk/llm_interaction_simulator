...
def plot_coefficients(ax, df, title, ylabel=False):
    ...
    ax.set_xlim([-0.5, 0.5])  
    ax.set_title(title)  # Increased title font size
    ax.set_xlabel('Effect on % Toxic Msgs')  # Increased x-axis label size

    if ylabel:
        ax.set_yticks(range(len(renamed_variables)))
        ax.set_yticklabels(renamed_variables.values)  # Increased y-tick labels size
        ax.set_ylabel('')
    else:
        ax.set_yticks(range(len(renamed_variables)))
    ...
    ax.tick_params(axis='both', which='major')  # Increase tick label sizes
...
fig.savefig('toxygen_regression_output_percentage_fractional.pdf')
...
plt.tight_layout()
# Save the figure as a PDF file
fig.savefig('toxygen_regression_output_scores_fractional.pdf')
plt.show()
